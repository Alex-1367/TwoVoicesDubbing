...
// Configuration
const INPUT_CSV = 'reversy/En-A.csv'; // Changed to match your file
...
async function parseCSVFromFile(filePath) {
    try {
        ...
        const startIndex = lines[0].toLowerCase().startsWith('english') ? 1 : 0;
        ...
        for (let i = startIndex; i < lines.length; i++) {
            ...
            if (separatorIndex === -1) {
                ...
                if (theIndex !== -1) {
                    const english = line.substring(0, theIndex).trim();
                    const german = line.substring(theIndex + 1).trim();
                    
                    // Clean the German text
                    const cleanedGerman = cleanGermanText(german);
                    
                    // Also clean English text
                    const cleanedEnglish = cleanGermanText(english);
                ...
                const english = line.substring(0, separatorIndex).trim();
                const german = line.substring(separatorIndex + 1).trim();
...
async function combineAudio(englishFile, silenceFile, germanFile, outputFile) {
    const command = `ffmpeg -i "concat:${englishFile}|${silenceFile}|${germanFile}" -c copy "${outputFile}" -y`;
...
async function generateAudioForWord(vocabItem, index) {
    const { german, english, originalEnglish } = vocabItem;
    
    console.log(`Processing ${index + 1}: "${english}" (to: "${german}")`);
    ...
    try {
        ...
        await combineAudio(englishFile, silenceFile, germanFile, outputFile);
        
        console.log(`✓ Created: ${path.basename(outputFile)}`);
        return { success: true, file: outputFile, german, english, originalEnglish };
    } catch (error) {
        console.error(`✗ Failed: "${german.substring(0, 30)}..."`, error.message);
        return { success: false, error: error.message, german, english, originalEnglish };
    } finally {
        // Clean up temp files
        try {
            await Promise.all([
...
export async function generateAudioFromCSV(csvFilePath = INPUT_CSV) {
    console.log('🎧 English-German Audio Generator (Cleaned Version)\n');
